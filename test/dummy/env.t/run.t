This test verify different environnement specifications that could be used with `opam-bundle`.
...
are available on linux os (`foo` not included).
  ...
  $ tar xvf baz-bundle.tar.gz | grep -v sha256 | sort
  baz-bundle/
...
Bundle package `baz`, with empty constraint specified in '--environment' option. That shouldn't filter any packages (considering 'any' constaint) and install
all dependencies.
